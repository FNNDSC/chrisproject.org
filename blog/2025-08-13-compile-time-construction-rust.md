...
initialization is a pure, meaning use of `LazyLock` makes your program
...
the runtime cost of `LazyLock` is entirely negligible. Nonetheless, if you are
a perfectionist like me, this annoys you to no end. Hence, we are motivated to
...
`const`-compatible (i.e. to be able to run at compile-time).
...
to use (since IDEs and `rust-analyzer` are not good at handling them).
...
Unlike `const fn`, macros can do almost anything, including iteration and I/O.
However, macros are difficult to write. They bypass Rust's type-safety and
compile-time guarantees. The tiny example above demonstrates how macros are not
type-safe: `src/lib.rs` compiles, yet it does not handle unexpected input, so
`sum!("lol")` will cause it to panic with an unhelpful error message. Neither
does macro compilation guarantee that the macro produces sound code.
